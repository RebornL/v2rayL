...
# success install info: V2Ray v4.22.1 is installed.
# fail install info: Failed to copy V2Ray binary and resources. or others
...
copyFile() {
    NAME=$1
    ERROR=`cp "${VSRC_ROOT}/${NAME}" "/usr/bin/v2rayL/${NAME}" 2>&1`
    if [[ $? -ne 0 ]]; then
...
makeExecutable() {
    chmod +x "/usr/bin/v2rayL/$1"
}

installV2Ray(){
    # Install V2Ray binary to /usr/bin/v2ray
    systemctl stop v2rayL
    mkdir -p /usr/bin/v2rayL
    copyFile v2ray
    if [[ $? -ne 0 ]]; then
    ...
    if [[ ! -f "/etc/v2rayL/config.json" ]]; then
        mkdir -p /etc/v2rayL
        mkdir -p /var/log/v2rayL
        cp "${VSRC_ROOT}/vpoint_vmess_freedom.json" "/etc/v2rayL/config.json"
        if [[ $? -ne 0 ]]; then
        ...
        sed -i "s/10086/${PORT}/g" "/etc/v2rayL/config.json"
        sed -i "s/23ad6b10-8d1a-40f7-8ad0-e3e35cd38297/${UUID}/g" "/etc/v2ray/config.json"

        colorEcho ${BLUE} "PORT:${PORT}"
        colorEcho ${BLUE} "UUID:${UUID}"
    fi
    systemctl start v2rayL
    return 0
}
